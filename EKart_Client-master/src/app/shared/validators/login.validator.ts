...
export class LoginValidators {
    static validateDealStartDate(control: AbstractControl): any {
      const dealStartDate = new Date(control.value);
      const currentDate = new Date();
      const monthDate = new Date(currentDate);
      monthDate.setMonth(monthDate.getMonth() + 1);
      if (dealStartDate > currentDate && dealStartDate <= monthDate) {
        return null;
      }
      return { 'dealStartDateError': true };
    }

  static validateDealEndDate(control: AbstractControl): any {
    return (dealStartDateControl: AbstractControl) => {
      if (control.value === dealStartDateControl.value) {
        return null;
      }
      return {'dealEndDateError': true};
    };
  }
